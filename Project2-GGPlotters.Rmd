...
```{r}
cat_vector <- c("NEIGHBORHOOD", "BUILDING.CLASS.CATEGORY", "TAX.CLASS.AT.PRESENT", "BLOCK", "TAX.CLASS.AT.TIME.OF.SALE", "ZIP.CODE", "GROSS.SQUARE.FEET", "SEASON", "LOT", "MONTH.BOUGHT","AGE")
...
adaboost <- boosting(high.end ~ ., data=bc_train, boos = FALSE, mfinal=40)
```

```{r}
p <- predict(adaboost,test_selected)
predictions <- as.data.frame(predict(adaboost,test_selected,type="class"))
